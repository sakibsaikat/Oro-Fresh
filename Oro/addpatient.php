<?php
    ...
    $sql = "INSERT INTO patients(name,age,sex,contact,area,reference,medical_history,treatment,problem,past_history,radiology_history,pain_on,upper_left,upper_right,lower_left,lower_right,BDR,BDC,problem_summary,total,due,next_visit,appointmen_date,prescription) VALUES ('{$name}','{$age}','{$sex}','{$contact}','{$area}','{$referrence}','{$medical_history}','{$treatment}','{$problems}','{$past_case_history}','{$radiological_history}','{$pain_on}','{$upper_left}','{$upper_right}','{$low_left}','{$low_right}','{$bdr}','{$bdc}','{$final_problem}','{$total_cost}','{$due}','{$next_visit}','{$ap_date}','{$pres_name}')";
